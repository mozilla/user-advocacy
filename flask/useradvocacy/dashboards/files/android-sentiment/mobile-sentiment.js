...
d3.json("/data/static/json/mobile-annotations.json", function(note_data) {
    ...
    d3.json("/data/api/v1/stats?measures=play_average&period_delta=90&date_end=2014-10-12&products=Firefox for Android&channels=all", function(data) {
        final_data = [
            {
                name: "Input rating",
                values: []
        ...
        for(i = 0; i < data.results.length; i++){
            ...
            item.value = data.results[i].play_average;
    ...
    d3.json("/data/api/v1/stats?measures=input_average,input_volume,adis&period_delta=90&date_end=2014-10-12&products=Firefox for Android&channels=all", function(data) {
...
d3.json("/data/api/v1/stats?measures=play_average&version_start=23&version_end=32&products=Firefox for Android&channels=release", function(data) {
    results = data.results;
    d3.select("#chart3")
        .datum(results)
...
d3.json("/data/api/v1/stats?measures=sumo_inproduct_visits&period=week&products=Firefox for Android&period_delta=12&date_end=2014-10-11&channels=all", function(data) {
    results = data.results;
    d3.select("#chart4")
        .datum(results)
