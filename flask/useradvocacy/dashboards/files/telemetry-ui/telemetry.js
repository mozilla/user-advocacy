...
url = '/data/api/v1/telemetry?' + param_string;
console.log(url);
...
//d3.json("hellofeedback.json",function (error, json) {
    ...
    results = _.map(json.results, function (i) {
        ...
        var out_users = _(users).map( function (j) {
    ...
    results = _(results)
        .map(function (i) {
            ...
            measure_name = i.alias?i.alias:i.measure;
            var summary = [];
            if (diff) {
...
// POLISH
    measure_name.style('color', function(d) {
        var bg = $(this).css('background-color');
        return (d3.hsl(bg).l < 0.6)?'white':null;
    });
    cells.style('color', function(d) {
        var bg = $(this).css('background-color');
        return (d3.hsl(bg).l < 0.6)?'white':null;
    });
    
    cells.style('padding', function(d) {
