...
CREATE TABLE tmp_mobile_input_base AS 
SELECT
...
-- Get 7 day info
CREATE TEMPORARY TABLE tmp_mobile_input AS 
SELECT
    t0.`date`,
    t0.version,
    t0.input_average,
...
FROM
    tmp_mobile_input_base t0
    LEFT JOIN tmp_mobile_input_base t1 ON(t0.d1 = t1.d0 AND t0.version = t1.version )
    LEFT JOIN tmp_mobile_input_base t2 ON(t0.d2 = t2.d0 AND t0.version = t2.version )
    LEFT JOIN tmp_mobile_input_base t3 ON(t0.d3 = t3.d0 AND t0.version = t3.version )
    LEFT JOIN tmp_mobile_input_base t4 ON(t0.d4 = t4.d0 AND t0.version = t4.version )
    LEFT JOIN tmp_mobile_input_base t5 ON(t0.d5 = t5.d0 AND t0.version = t5.version )
...
-- Get SUMO data
CREATE TEMPORARY TABLE tmp_mobile_sumo AS 
SELECT
...
CREATE TEMPORARY TABLE tmp_mobile_base AS 
SELECT
    dates.`date`  AS `date`,
...
FROM
    release_info ri
    JOIN (SELECT `date` FROM tmp_mobile_input input GROUP BY 1) dates
GROUP BY 1,2,3
;
